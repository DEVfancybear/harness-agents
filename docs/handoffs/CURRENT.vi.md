...
**Cập nhật:** 23/09/2026 · **Assignment:** M7–M12 theo kế hoạch `implementation-next`, tuần tự theo dependency và gate từng checkpoint; commit+push sau mỗi action. **Checkpoint hiện tại:** M7 (`e5ef55c`), M8 (`32c3400`), M9 (`4035f8b`), M10 (`0a1167d`), **M11-01** (`90bd0f6`/`3915b5f`/`ada1917`) xong + gate xanh; **M11-02** (`9d55914`) xong; **M11-03** (`5366b1e`) xong + **gate M11 lần ba xanh** (digest `sha256:b2c5e98f14ac95cc7dc4b3dc69dc3910870ea33c5a6a67ba8581a5dabc18713b`, `391` file, **5 required test**). Cộng một fix ngoài scope nhưng chặn gate: `test(m2)` (`220e687`) — assertion `a07_401` nay khẳng định trên attempt record bền. **M11-04 chưa làm** (xem §7e). **Milestone còn lại: hoàn tất M11-04, rồi M12** (prerequisites M4).
...
## 7e. M11 — M11-01/02/03 xong (KHÔNG được đọc là "xong milestone")

- **Xong + gate xanh:** M11-01 (daemon host + IPC, hai test daemon), M11-02 (schedules/occurrences, A34), **M11-03 (external task driver + polling, A35)**. Registry: M11-01/02/03 = `implemented_unverified`, **A35 = implemented**; required test của M11 = **5**, tất cả bằng qualified selector nên gate tự discover + tự chạy. Gate M11 lần ba: `sha256:b2c5e98f14ac95cc7dc4b3dc69dc3910870ea33c5a6a67ba8581a5dabc18713b`, **5/5 required**, `workspace-tests` `passed ngay lần đầu, **không retry**`.
...
- **Bug M11-03 đáng nhớ nhất:** tôi phân loại JSON-RPC error là `Definite` ("chắc chắn không apply"). Negative control chứng minh **ngược lại**: guard SEP-2663 của SDK tạo task **rồi** mới trả error ⇒ fixture đã tạo task dù client nhận error. Luật đúng: chỉ refusal **trước khi gửi** là `Definite`, mọi thất bại sau khi đã gửi là `Ambiguous`. Bug thứ hai: `recover_external_jobs` không báo job đã `ambiguous` từ trước.
- **Fix ngoài scope M11, có lý do đo được:** `test(m2)` — `a07_401` đỏ trong gate vì loopback response bị mất (đo **5/30** lần) được adapter retry **đúng theo thiết kế**, làm assertion "attempted once" đọc sai. Nay khẳng định trên **provider attempt record** (không attempt nào sau attempt fail bằng authority error) + retry **chỉ khi** evidence nói loopback mất response. Stress sau sửa: **25/25** xanh.
- **Chưa làm:** **M11-04** (non-interactive policy + notification outbox). Ngoài ra chưa có CLI cho external job (consumer là daemon), chưa hỗ trợ MCP remote transport.
- **M11 chưa nằm trong matrix CI** (`ci.yml` mới M0–M9) ⇒ chưa có bằng chứng Linux. Flake loopback của host vẫn tồn tại (đặc tính máy, không phải code).
...
**Hoàn tất M11 rồi mới sang M12.** Việc cụ thể, theo thứ tự dependency: (1) ~~**M11-01** daemon host + IPC~~, ~~**M11-02** schedules/occurrences (A34)~~, ~~**M11-03** external task driver + polling (A35)~~ — **xong, gate xanh, required test**; (2) **M11-04** non-interactive policy + notification outbox: schedule **không** mang auto-approve (đã có trường + test), occurrence cần grant ⇒ **waiting** + câu hỏi bền (dùng bảng `questions` M3) với expiry, approve ⇒ `launched` **một lần**, deny ⇒ `skipped`, hết hạn ⇒ `expired`; `status` phải hiện waiting + expiry + next action; **outbox** bền dedupe theo `(subject, change_digest)` + backoff bounded, và **không gửi gì** khi chưa cấu hình connector ⇒ test `milestone_m11_policy.rs`; (3) thêm M11 vào matrix CI `verify-milestones`; (4) chạy lại gate M11; (5) evidence + handoff; (6) commit + push. **Sau đó M12** (`M12.vi.md`, prerequisites **M4**, acceptance A36 strict confinement).
## 9. Blocked on
