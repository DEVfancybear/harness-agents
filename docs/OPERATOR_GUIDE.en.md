...
| Group | Commands |
| --- | --- |
| Help and state | `/help`, `/hotkeys`, `/session` (`/status`), `/config`, `/model [search]`, `/effort [level]` (`/thinking`), `/cost`, `/context` (`/usage`), `/system-prompt`, `/permissions`, `/hooks`, `/mcp`, `/agents`, `/skills` |
| Session and answer | `/new` (`/clear` also clears the viewport), `/resume [id]`, `/name [name]` (`/rename`), `/more`, `/compact [instructions]`, `/export [path]`, `/copy`, `/quit` (`/exit`) |
