...
3. **Durability.** `foreign_keys=ON`, `busy_timeout=5s`, `journal_mode=WAL`, `synchronous=NORMAL` cho writer (sửa 2026-09-26, trước đây `FULL`); read-only mở không tạo file, không migrate. ACK chỉ được trả **sau** `COMMIT`. Với WAL, `NORMAL` giữ commit khi process bị crash hay bị kill; chỉ lỗi hệ điều hành hoặc mất điện mới có thể làm mất vài commit cuối, và database vẫn nhất quán. `FULL` fsync WAL ở mọi commit, tức ba lần cho mỗi tool call, và là nguyên nhân chính làm tool call chậm.
