...
3. **Durability.** `foreign_keys=ON`, `busy_timeout=5s`, `journal_mode=WAL`, `synchronous=NORMAL` for a writer (amended 2026-09-26, was `FULL`); a read-only open creates nothing and migrates nothing. An ACK is returned only **after** `COMMIT`. Under WAL, `NORMAL` keeps a commit through a crash or kill of the process; only an operating-system crash or power loss can drop the last commits, and the database stays consistent. `FULL` fsynced the WAL on every commit, three times per tool call, and was the main cost of a tool call.
