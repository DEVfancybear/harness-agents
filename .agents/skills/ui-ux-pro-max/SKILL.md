...
If `python` is not found, try `python3`, then `py -3`. Requires Python 3.x, no external dependencies. If Python is missing, do not install it yourself: ask the user to install Python 3, and until then rely on `references/quick-reference.md`.
...
```bash
python "<skill-directory>/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python "<skill-directory>/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python "<skill-directory>/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python "<skill-directory>/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python "<skill-directory>/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python "<skill-directory>/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python "<skill-directory>/scripts/search.py" "<keyword>" --stack <stack>
```
...
python "<skill-directory>/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python "<skill-directory>/scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python "<skill-directory>/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
