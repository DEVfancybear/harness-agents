...
# Windows paths are limited to 260 characters. pnpm nests packages deep under
# node_modules/.pnpm, and Parcel then fails with ENOENT on a "\\?\" path when the
# project itself sits in a long directory. Refuse early with a clear message.
case "$OSTYPE" in
  msys*|cygwin*|win32*)
    TARGET_PATH="$(pwd -W 2>/dev/null || pwd)/$PROJECT_NAME"
    if [ "${#TARGET_PATH}" -gt 110 ]; then
      echo "❌ Error: the project path is ${#TARGET_PATH} characters long: $TARGET_PATH"
      echo "   On Windows, bundling fails once node_modules paths pass 260 characters."
      echo "   Create the project in a shorter directory, for example C:/work/$PROJECT_NAME."
      exit 1
    fi
    ;;
esac
...
# The template's icon link (vite.svg in older templates, favicon.svg in newer
# ones) points at a file Parcel cannot resolve from the root; the artifact has
# no icon.
