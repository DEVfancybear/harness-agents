...
Status at the time of writing (HA_LAUNCH H07):
  - i01 passes here: bare `ha` opens the app, renders its header and exits 0;
  - i06 fails on the bracketed-paste expectation;
  - i07 hangs the harness (killed by the timeout below), so Ctrl-C delivery through
    ConPTY is still unproven.

.PARAMETER Filter
Test-name filter passed to the test binary. Defaults to all three PTY cases.
